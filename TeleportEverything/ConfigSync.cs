    ...
    public class ConfigSync
    {
        ...
        public bool IsLocked
        {
            get => (forceConfigLocking ?? lockedConfig != null &&
                ((IConvertible)lockedConfig.BaseConfig.BoxedValue).ToInt32(CultureInfo
                    .InvariantCulture) != 0) && !lockExempt;
        ...
        static ConfigSync()
        {
            ...
            {
                if (PatchProcessor.GetPatchInfo(AccessTools.DeclaredMethod(typeof(ZNet), "Awake"))
                        ?.Postfixes
                        .Count(p => p.PatchMethod.DeclaringType == typeof(RegisterRPCPatch)) > 0)
                {
                    return;
                }

                Harmony harmony = new("org.bepinex.helpers.ServerSync");
                foreach (var type in typeof(ConfigSync).GetNestedTypes(BindingFlags.NonPublic)
                             .Where(t => t.IsClass))
                {
        ...
        public SyncedConfigEntry<T> AddConfigEntry<T>(ConfigEntry<T> configEntry)
        {
            if (configData(configEntry) is not SyncedConfigEntry<T> syncedEntry)
            {
                ...
                AccessTools.DeclaredField(typeof(ConfigDescription), "<Tags>k__BackingField")
                    .SetValue(configEntry.Description,
                        new object[] { new ConfigurationManagerAttributes() }
                            ...
                            .Concat(new[] { syncedEntry }).ToArray());
        ...
        {
            if (allCustomValues.Select(v => v.Identifier)
                .Concat(new[] { "serverversion", "requiredversion" })
                .Contains(customValue.Identifier))
            {
                throw new Exception(
                    "Cannot have multiple settings with the same name or with a reserved name (serverversion or requiredversion)");
            }
            ...
            private static void Postfix(ZNet __instance)
            {
                ...
                foreach (var configSync in configSyncs)
                {
                    ...
                    ZRoutedRpc.instance.Register<ZPackage>(configSync.Name + " ConfigSync",
                        configSync.RPC_ConfigSync);
                    if (isServer)
                    {
                        Debug.Log(
                            $"Registered '{configSync.Name} ConfigSync' RPC - waiting for incoming connections");
                    }
                }

                IEnumerator WatchAdminListChanges()
                {
                    var adminList = (SyncedList)AccessTools
                        .DeclaredField(typeof(ZNet), "m_adminList").GetValue(ZNet.instance);
                        ...
                        if (!adminList.GetList().SequenceEqual(CurrentList))
                        {
                            ...
                            void SendAdmin(List<ZNetPeer> peers, bool isAdmin)
                            {
                                var package = ConfigsToPackage(packageEntries: new[]
                                {
                                    new PackageEntry
                                    {
                                        section = "Internal", key = "lockexempt",
                                        type = typeof(bool), value = isAdmin
                                    }
                                });
                                ...
                                if (configSyncs.First() is { } configSync)
                                {
                                    ZNet.instance.StartCoroutine(
                                        configSync.sendZPackage(peers, package));
                                }
                            }

                            List<ZNetPeer> adminPeer = ZNet.instance.GetPeers()
                                .Where(p => adminList.Contains(p.m_rpc.GetSocket().GetHostName()))
                                .ToList();
                            List<ZNetPeer> nonAdminPeer =
                                ZNet.instance.GetPeers().Except(adminPeer).ToList();
        ...
        private static class RegisterClientRPCPatch
        {
            private static void Postfix(ZNet __instance, ZNetPeer peer)
            {
                if (!__instance.IsServer())
                {
                    foreach (var configSync in configSyncs)
                    {
                        peer.m_rpc.Register<ZPackage>(configSync.Name + " ConfigSync",
                            configSync.RPC_InitialConfigSync);
                    }
                }
            }
        }
        ...
        private readonly List<KeyValuePair<long, string>>
            cacheExpirations = new(); // avoid leaking memory
            ...
            try
            {
                if (isServer && IsLocked)
                {
                    var exempt =
                        ((SyncedList?)AccessTools.DeclaredField(typeof(ZNet), "m_adminList")
                            .GetValue(ZNet.instance))?.Contains(SnatchCurrentlyHandlingRPC
                            .currentRpc?.GetSocket()?.GetHostName());
                    if (exempt == false)
                    {
                        return;
                    }
                ...
                if ((packageFlags & FRAGMENTED_CONFIG) != 0)
                {
                    ...
                    if (!configValueCache.TryGetValue(cacheKey, out var dataFragments))
                    {
                        ...
                        cacheExpirations.Add(
                            new KeyValuePair<long, string>(DateTimeOffset.Now.AddSeconds(60).Ticks,
                                cacheKey));
                    }
                ...
                {
                    Debug.Log(
                        $"Received {configs.configValues.Count} configs and {configs.customValues.Count} custom values from the server for mod {DisplayName ?? Name}");
        ...
        private ParsedConfigs ReadConfigsFromPackage(ZPackage package)
        {
            ...
            Dictionary<string, OwnConfigEntryBase> configMap = allConfigs
                .Where(c => c.SynchronizedConfig).ToDictionary(
                    c => c.BaseConfig.Definition.Section + "_" + c.BaseConfig.Definition.Key,
                    c => c);
            ...
            for (var i = 0; i < valueCount; ++i)
            {
                ...
                if (typeName == "" || type != null)
                {
                    ...
                    try
                    {
                        value = typeName == ""
                            ? null
                            : ReadValueWithTypeFromZPackage(package, type!);
                    }
                    ...
                    if (groupName == "Internal")
                    {
                        ...
                        else if (configName == "requiredversion")
                        {
                            ...
                            if (CurrentVersion == null ||
                                new System.Version(value?.ToString() ?? "0.0.0") >
                                new System.Version(CurrentVersion))
                            {
                                // ReSharper restore RedundantNameQualifier
                                Debug.LogError(
                                    $"Received minimum version is higher than required version: minimum required version = {value?.ToString() ?? "0.0.0"}; local version = {CurrentVersion ?? "unknown"}");
                                Game.instance.Logout();
                                AccessTools.DeclaredField(typeof(ZNet), "m_connectionStatus")
                                    .SetValue(null, ZNet.ConnectionStatus.ErrorVersion);
                                connectionError =
                                    $"Mod {DisplayName ?? Name} requires minimum {value}. Installed is version {CurrentVersion}.";
                            }
                        }
                        ...
                        {
                            if (typeName == "" && (!config.Type.IsValueType ||
                                                   Nullable.GetUnderlyingType(config.Type) !=
                                                   null) || GetZPackageTypeString(config.Type) ==
                                typeName)
                            {
                    ...
                    else if (configMap.TryGetValue(groupName + "_" + configName, out var config))
                    {
                        ...
                        if (typeName == "" && (!expectedType.IsValueType ||
                ...
                {
                    Debug.LogWarning(
                        $"Got invalid type {typeName}, abort reading of received configs");
        ...
        private static bool isWritableConfig(OwnConfigEntryBase config)
        {
            ...
            return configSync.IsSourceOfTruth || !config.SynchronizedConfig ||
                   config.LocalBaseValue == null || !configSync.IsLocked &&
                   (config != configSync.lockedConfig || lockExempt);
        }
        ...
        private void serverLockedSettingChanged()
        {
            ...
            {
                configAttribute<ConfigurationManagerAttributes>(configEntryBase.BaseConfig)
                    .ReadOnly = !isWritableConfig(configEntryBase);
            }
        }
        ...
        private IEnumerator<bool> distributeConfigToPeers(ZNetPeer peer, ZPackage package)
        {
            ...
            IEnumerable<bool> waitForQueue()
            {
                ...
                while (peer.m_socket.GetSendQueueSize() > maximumSendQueueSize)
                {
                    ...
                    {
                        Debug.Log(
                            $"Disconnecting {peer.m_uid} after 30 seconds config sending timeout");
            ...
            if (package.GetArray() is { LongLength: > packageSliceSize } data)
            {
                ...
                for (var fragment = 0; fragment < fragments; ++fragment)
                {
                    ...
                    fragmentedPackage.Write(data.Skip(packageSliceSize * fragment)
                        .Take(packageSliceSize).ToArray());
        ...
        private IEnumerator sendZPackage(long target, ZPackage package)
        {
            ...
            var peers = (List<ZNetPeer>)AccessTools.DeclaredField(typeof(ZRoutedRpc), "m_peers")
        ...
        private IEnumerator sendZPackage(List<ZNetPeer> peers, ZPackage package)
        {
            ...
            if (package.GetArray() is { LongLength: > compressMinSize } rawData)
            {
                ...
                using (DeflateStream deflateStream =
                       new(output, System.IO.Compression.CompressionLevel.Optimal))
                {
            ...
            var writers = peers.Where(peer => peer.IsReady())
        ...
        private class SendConfigsAfterLogin
        {
            private class BufferingSocket : ISocket
            {
                ...
                public void GetConnectionQuality(out float localQuality, out float remoteQuality,
                    ...
                    Original.GetConnectionQuality(out localQuality, out remoteQuality, out ping,
                        out outByteSec, out inByteSec);
                ...
                public void Send(ZPackage pkg)
                {
                    ...
                    if ((methodHash == "PeerInfo".GetStableHashCode() ||
                         methodHash == "RoutedRPC".GetStableHashCode()) && !finished)
                    {
                        Package.Add(
                            new ZPackage(
                                pkg.GetArray())); // the original ZPackage gets reused, create a new one
                    }
                    else
                    {
                        Original.Send(pkg);
            ...
            private static void Prefix(ref Dictionary<Assembly, BufferingSocket>? __state,
                ZNet __instance, ZRpc rpc)
            {
                if (__instance.IsServer())
                {
                    BufferingSocket bufferingSocket = new(rpc.GetSocket());
                    AccessTools.DeclaredField(typeof(ZRpc), "m_socket")
                        .SetValue(rpc, bufferingSocket);
            ...
            private static void Postfix(Dictionary<Assembly, BufferingSocket> __state,
                ZNet __instance, ZRpc rpc)
            {
                if (!__instance.IsServer())
                {
                ...
                void SendBufferedData()
                {
                    ...
                    {
                        AccessTools.DeclaredField(typeof(ZRpc), "m_socket")
                            .SetValue(rpc, bufferingSocket.Original);
                    }

                    bufferingSocket = __state[Assembly.GetExecutingAssembly()];
                    bufferingSocket.finished = true;

                    foreach (var package in bufferingSocket.Package)
                    {
                ...
                IEnumerator sendAsync()
                {
                    foreach (var configSync in configSyncs)
                    {
                        ...
                        if (configSync.MinimumRequiredVersion != null)
                        {
                            entries.Add(new PackageEntry
                            {
                                section = "Internal", key = "requiredversion",
                                type = typeof(string), value = configSync.MinimumRequiredVersion
                            });
                        }
                        ...
                        entries.Add(new PackageEntry
                        {
                            ...
                            value = ((SyncedList)AccessTools
                                    .DeclaredField(typeof(ZNet), "m_adminList")
                                    .GetValue(ZNet.instance))
                                .Contains(rpc.GetSocket().GetHostName())
                        });

                        var package = ConfigsToPackage(
                            configSync.allConfigs.Select(c => c.BaseConfig),
        ...
        private static OwnConfigEntryBase? configData(ConfigEntryBase config) => config.Description
            .Tags?.OfType<OwnConfigEntryBase>().SingleOrDefault();

        public static SyncedConfigEntry<T>? ConfigData<T>(ConfigEntry<T> config) =>
            config.Description.Tags?.OfType<SyncedConfigEntry<T>>().SingleOrDefault();

        private static T configAttribute<T>(ConfigEntryBase config) =>
            config.Description.Tags.OfType<T>().First();
        ...
        private static class PreventSavingServerInfo
        {
            private static bool Prefix(ConfigEntryBase __instance, ref string __result)
            {
                ...
                __result =
                    TomlTypeConverter.ConvertToString(data.LocalBaseValue, __instance.SettingType);
                return false;
            }
        }
        ...
        private static class PreventConfigRereadChangingValues
        {
            private static bool Prefix(ConfigEntryBase __instance, string value)
            {
                ...
                {
                    data.LocalBaseValue =
                        TomlTypeConverter.ConvertToValue(value, __instance.SettingType);
                }
                catch (Exception e)
                {
                    Debug.LogWarning(
        ...
        private static ZPackage ConfigsToPackage(IEnumerable<ConfigEntryBase>? configs = null,
            ...
            IEnumerable<PackageEntry>? packageEntries = null, bool partial = true)
        {
            var configList =
                configs?.Where(config => configData(config)!.SynchronizedConfig).ToList() ??
            ...
            package.Write(configList.Count + customValueList.Count +
                          (packageEntries?.Count() ?? 0));
            ...
            foreach (var config in configList)
            {
                AddEntryToPackage(package,
                    new PackageEntry
                    {
                        section = config.Definition.Section, key = config.Definition.Key,
                        type = configType(config), value = config.BoxedValue
                    });
            }
        ...
        private static void AddValueToZPackage(ZPackage package, object? value)
        {
            ...
            if (value is Enum)
            {
                value = ((IConvertible)value).ToType(Enum.GetUnderlyingType(value.GetType()),
        ...
        private static object ReadValueWithTypeFromZPackage(ZPackage package, Type type)
        {
            if (type is { IsValueType: true, IsPrimitive: false, IsEnum: false })
            {
                ...
                foreach (var field in fields)
                {
                    ...
                    if (typeName != GetZPackageTypeString(field.FieldType))
                    {
                        throw new InvalidDeserializationTypeException
                        {
                            received = typeName, expected = GetZPackageTypeString(field.FieldType),
                            field = field.Name
                        };
                    }
            ...
            if (type != typeof(List<string>) && type.IsGenericType &&
                typeof(ICollection<>).MakeGenericType(type.GenericTypeArguments[0]) is
                    { } collectionType &&
            ...
            var param =
                (ParameterInfo)FormatterServices.GetUninitializedObject(typeof(ParameterInfo));
